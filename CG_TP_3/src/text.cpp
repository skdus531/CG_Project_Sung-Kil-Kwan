...
bool init_text()
{
	...
	VAO = cg_create_vertex_array(vertex_buffer);
	if (!VAO) { printf("%s(): VAO==nullptr\n", __func__); return false; }

	return true;
}

void render_text(std::string text, float _x, float _y, GLfloat scale, vec4 color, GLfloat dpi_scale)
{
	// apply dpi_scale
	if (dpi_scale > 1.0f)
	{
		_x = float(_x * dpi_scale);
		_y = float(_y * dpi_scale);
		scale *= dpi_scale;
	}
	...
	//glUniform1i(glGetUniformLocation(program_text, "text"), 1);
