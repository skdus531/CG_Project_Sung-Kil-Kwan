...
static const char* image_path = "../bin/images/particle.png";
...
void particle_update( vec4 color) {
	for (auto& p : particles) p.update(color,t);
	...
	uloc = glGetUniformLocation(program, "p_aspect_matrix");	if (uloc > -1) glUniformMatrix4fv(uloc, 1, GL_TRUE, aspect_matrix);
}
...
void update()
{
	...
	particle_update(vec4(Student[0].color, 1.0f));
	...
	if (gs.game_start) {
		...
		if (gs.user_score >= 500) {  //stage 5 다 클리어 
			cam.init();
			...
			if (PEngine.collision_exists(Student[0], *p)) {
				if (p->type == 0) { // 시험: 맞는 색깔로
					if (p->subject_id == Student[0].subject_id) {
						...
						//	//p.set(Student[0].position, vec4(Student[0].color, 1.0f));
						//	particle_t p;
						//	p.color = vec4(Student[0].color, 1.0f);
						//	p.pos = Student[0].position;
...
void render()
{
	...
	if (!gs.game_start && !gs.title && gs.how_to) {
		...
		render_text("Press C to change your color.", 40, 230, 0.45f, vec4(1.0f, 1.0f, 1.0f, 1.0f), dpi_scale);
		...
		render_text("Press Z to throw your pen.", 40, 480, 0.45f, vec4(1.0f, 1.0f, 1.0f, 1.0f), dpi_scale);
...
bool particle_init() {
	...
	image* img = cg_load_image(image_path, true); if (!img) return false;
